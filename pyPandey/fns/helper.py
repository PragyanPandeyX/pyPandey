...
def ErrInfo(exception, full=False):
    ...
    try:
        from pyPandey.startup import LOGS
    except ImportError:
        print("IMPORT ERROR")
...
if run_as_module:
    ...
    async def safeinstall(event):
        ...
        if plug in HELP:
            ...
            for i in HELP[plug]:
                output += i
            output += "\n© @TeamPandey"
            await eod(ok, f"✓ `Pandey - Installed`: `{plug}` ✓\n\n{output}")
    ...
    def gen_chlog(repo, diff):
        ...
        ch_log = tldr_log = ""
        ch = f"<b>Pandey {Pragyan_version} updates for <a href={UPSTREAM_REPO_URL}/tree/{ac_br}>[{ac_br}]</a>:</b>"
        ch_tl = f"Pandey {Pragyan_version} updates for {ac_br}:"
        d_form = "%d/%m/%y || %H:%M"
