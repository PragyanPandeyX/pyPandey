...
class MongoDB(_BaseDatabase):
    def __init__(self, key, dbname="pyPandeyUB"):
        self.dB = MongoClient(key, serverSelectionTimeoutMS=5000)
        self.db = self.dB[dbname]
        super().__init__()

    def __repr__(self):
        return f"<Pandey.MonGoDB\n -total_keys: {len(self.keys())}\n>"
    ...
    def flushall(self):
        self.dB.drop_database("PandeyDB")
        self._cache.clear()
        return True
...
# for SQL Implementation in Pandey.
#
...
class SqlDB(_BaseDatabase):
    ...
    def usage(self):
        self._cursor.execute(
            "SELECT pg_size_pretty(pg_relation_size('Pandey')) AS size"
        )
    ...
    def get(self, variable):
        try:
            self._cursor.execute(f"SELECT {variable} FROM Pandey")
    ...
    def set(self, key, value):
        try:
            self._cursor.execute(f"ALTER TABLE Pandey DROP COLUMN IF EXISTS {key}")
        except (psycopg2.errors.UndefinedColumn, psycopg2.errors.SyntaxError):
            pass
        except BaseException as er:
            LOGS.exception(er)
        self._cache.update({key: value})
        self._cursor.execute(f"ALTER TABLE Pandey ADD {key} TEXT")
        self._cursor.execute(f"INSERT INTO Pandey ({key}) values (%s)", (str(value),))
        return True

    def delete(self, key):
        try:
            self._cursor.execute(f"ALTER TABLE Pandey DROP COLUMN {key}")
        except psycopg2.errors.UndefinedColumn:
            return False
        return True
