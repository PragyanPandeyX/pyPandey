...
from pyPandey._misc._assistant import asst_cmd
from pyPandey.dB.gban_mute_db import is_gbanned
from pyPandey.fns.tools import get_chat_and_msgid
...
from pyPandey.fns.info import get_chat_info
...
async def telegraphcmd(event):
    xx = await event.eor(get_string("com_1"))
    match = event.pattern_match.group(1).strip() or "Pandey"
    reply = await event.get_reply_message()
    if not reply:
...
async def get_restricted_msg(event):
    ...
    if not (chat and msg):
        return await event.eor(
            "Invalid link!\nEg: `https://t.me/TeamPandey/3` or `https://t.me/c/1313492028/3`"
        )
