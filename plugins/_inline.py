...
from pyPandey._misc._assistant import callback, in_pattern
from pyPandey.dB._core import HELP, LIST
from pyPandey.fns.helper import gen_chlog, time_formatter, updater
from pyPandey.fns.misc import split_list
...
SUP_BUTTONS = [
    [
        Button.url("• Repo •", url="https://github.com/TeamPandey/Pandey"),
        Button.url("• Support •", url="t.me/PandeySupportChat"),
    ],
]
...
async def inline_handler(event):
    ...
    if inline_pic():
        ...
        result = await event.builder.article(
            title="Pandey Help Menu", text=text, buttons=_main_help_menu
        )
...
async def ibuild(e):
    ...
    if pic:
        try:
            ...
            LOGS.exception(er)
    result = [
        await builder.article("Pandey Op", text=txt, link_preview=False, buttons=btn)
    ]
