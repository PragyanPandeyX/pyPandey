...
from . import Redis, eor, get_string, pdB, Pragyan_cmd


@Pragyan_cmd(pattern="setdb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@Pragyan_cmd(pattern="deldb( (.*)|$)", fullsudo=True)
async def _(ult):
...
@Pragyan_cmd(pattern="rendb( (.*)|$)", fullsudo=True)
async def _(ult):
    match = ult.pattern_match.group(1).strip()
    if not match:
